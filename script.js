...
let displayElement = document.querySelector('#data');
let itemsPerPageInput = document.querySelector('#size');
let pageBtnParent = document.querySelector('#pages');
let index = 0;
...
let pageBtns = document.querySelectorAll('.page-btns');

const displayPage = () => {
  displayData(displayElement, pages[index]);
  displayPagination(pageBtnParent, pages, index);
};
...
pageBtnParent.addEventListener('click', (e) => {
...
function displayData(container, array) {
  displayHTML = '';
...
function displayPagination(container, pages, activeIndex) {
  let displayBtns = pages.map((_, pageIndex) => {
    return `<li class="page-item ${
      pageIndex === activeIndex ? 'active' : ''
    }" id="page${pageIndex}">
            <button class="page-link page-btns" data-id=${pageIndex}>${
      pageIndex + 1
    }</button>
          </li>`;
  });
  ...
  displayBtns.push(`<li class="page-item ${
    activeIndex === pages.length - 1 ? 'disabled' : ''
  }">
